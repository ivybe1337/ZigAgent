...
pub const AVAILABLE_COMMANDS = [_]SlashItem{
    .{ .cmd = "/model", .desc = "Switch active model (interactive picker or /model <name>)" },
    .{ .cmd = "/keys", .desc = "Auth vault: view readiness or set key (/keys <prov> <key>)" },
    .{ .cmd = "/settings", .desc = "Interactive settings cockpit (autonomy, sandbox, context)" },
    .{ .cmd = "/reset", .desc = "Clear conversation history & start fresh session" },
    .{ .cmd = "/doctor", .desc = "Deep system diagnostics & toolchain verification" },
    .{ .cmd = "/skills", .desc = "Domain skills & playbooks (/skills or /skills <name>)" },
    .{ .cmd = "/mcp", .desc = "Model Context Protocol servers & registered tools" },
    .{ .cmd = "/tools", .desc = "Inspect available autonomous tools (file, shell, web, vision)" },
    .{ .cmd = "/vision", .desc = "Spatial vision screen capture & computer-use grounding" },
    .{ .cmd = "/swarm", .desc = "Autonomous multi-agent consensus crew (/swarm <goal>)" },
    .{ .cmd = "/snapshot", .desc = "Time Machine state checkpoint (/snapshot [label|rollback])" },
    .{ .cmd = "/gui", .desc = "Launch Studio Web Desktop & Native macOS GUI" },
    .{ .cmd = "/help", .desc = "Show categorized studio command cheat sheet" },
    .{ .cmd = "/clear", .desc = "Clear terminal screen" },
    .{ .cmd = "/exit", .desc = "Cleanly exit ZigAgent CLI" },
};
